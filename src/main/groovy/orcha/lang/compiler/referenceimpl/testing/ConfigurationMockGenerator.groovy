...
class ConfigurationMockGenerator {
	...
	private JType forName(String className, JCodeModel codeModel){
		
		JType jType
		 
		if(className.contains("<") && className.contains(">")){
			String container = className.substring(0, className.indexOf("<"))
			JClass containerClass = codeModel.ref(Class.forName(container))
			String contained = className.substring(className.indexOf("<")+1, className.indexOf(">"))
			JClass containedClass = codeModel.ref(Class.forName(contained))
			jType = containerClass.narrow(containedClass)
		} else {
			jType = codeModel.ref(Class.forName(className))
		}
	...
	private JExpression newExpression(String className, JCodeModel codeModel){
		...
		if(className.contains("<") && className.contains(">")){
			...
			if(containerClass.isInterface()){		// interfaces can not been instancied with a new
				String canonicalName = containerClass.getCanonicalName() 
				if(canonicalName == "java.util.List"){
					containerClass = Class.forName("java.util.ArrayList")
				} else if(canonicalName == "java.util.Set"){
					containerClass = Class.forName("java.util.HashSet")
				} else if(canonicalName == "java.util.SortedSet"){
					containerClass = Class.forName("java.util.TreeSet")
				}
			}
			String contained = className.substring(className.indexOf("<")+1, className.indexOf(">"))
			Class containedClass = Class.forName(contained)
			jExpression = JExpr._new(codeModel.ref(containerClass).narrow(containedClass))
		} else {
			JClass jClasse = codeModel.ref(Class.forName(className))
			jExpression = JExpr._new(jClasse)
		}
		
		return jExpression
	}
		
	private String newASM(String className){
		...
		if(className.contains("<") && className.contains(">")){
		   ...
		   if(containerClass.isInterface()){		// interfaces can not been instancied with a new			   
			   if(canonicalName == "java.util.List"){
				   asmExpression = "java.util.ArrayList"
			   } else if(canonicalName == "java.util.Set"){
				   asmExpression = "java.util.HashSet"
			   } else if(canonicalName == "java.util.SortedSet"){
				   asmExpression = "java.util.TreeSet"
			   }
		   } else {
			   asmExpression = canonicalName
		   }
		} else {
			asmExpression = className
		}
	   		
		asmExpression = asmExpression.replaceAll("\\.", "/")
		
	    return asmExpression
	}
	
	private String argAndReturnOfService(String argClassName, String returnClassName){
		
		String asmExpression
		
		if(argClassName.contains("<") && argClassName.contains(">")){
		   String container = argClassName.substring(0, argClassName.indexOf("<"))
		   Class containerClass = Class.forName(container)
		   asmExpression = containerClass.getCanonicalName()   
		} else {
			asmExpression = argClassName
		}
		
		asmExpression = "(L" + asmExpression + ";)L" + returnClassName + ";"
	...
	private String invokeServiceAMS(String className){
